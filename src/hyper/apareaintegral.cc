...
ApAreaIntegral<REAL>::ApAreaIntegral(const std::string& name)
  : WxObject(name) {
}

template<typename REAL>
void
ApAreaIntegral<REAL>::setup(const WxCryptSet& wxc)
{
    _ninp = 0;
    if (wxc.has("InpRange"))
    {
...
template<typename REAL>
bool
ApAreaIntegral<REAL>::compAreaIntegral(REAL *tx, REAL *qfull, REAL *qauxfull, REAL *sfull)
{
    ...
    bool res = this->integral(_ninp, tx, &_inpValues[0], &_inpAuxValues[0], &_outValues[0]);
