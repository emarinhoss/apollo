#include "aptwofluidomfbc.h"
#include <wxmath.h>

template <typename REAL>
void
APTwoFluidOMFBC<REAL>::setup(const WxCryptSet& wxc, DM dm)
{
...
template <typename REAL>
void
APTwoFluidOMFBC<REAL>::applyBC(REAL *xc, REAL *nx, REAL *q, REAL *qaux, REAL *AreaInts, REAL *qBC)
{
    REAL x = xc[1];
    REAL y = xc[2];
    REAL r = sqrt(x*x+y*y);
//    REAL theta = atan(y/x);
    ...
    REAL Bx = Bt*sin(_omega*t+_phase);
