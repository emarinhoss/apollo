#ifndef WXPREADGMSHGRID_H
#define WXPREADGMSHGRID_H
...
void
CreateGmshGrid(MPI_Comm comm, PetscViewer viewer, PetscBool interpolate, DM *dm)
{
  FILE          *fd;
  ...
  if (!rank) {
    ...
    for (c = 0; c < numCells; ++c) {
      CreateGmsh_ReadElement(fd, &dim, &cellNum, &numCorners, cone, tags);
      if (dim > tdim) {
        tdim = dim;
        trueNumCells = 0;
      }
...
#undef __FUNCT__
#define __FUNCT__ "CreateGmsh_ReadElement"
void
CreateGmsh_ReadElement(FILE *fd, PetscInt *dim, int *cellNum, PetscInt *numCorners, int cone[], int tags[])
{
  PetscInt t;
