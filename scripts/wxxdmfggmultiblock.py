...
topology2d = r"""
   <Topology TopologyType="2DSMesh" NumberOfElements="%d %d 1"/>
"""

topology3d = r"""
   <Topology TopologyType="3DSMesh" NumberOfElements="%d %d %d 1"/>
"""
    ...
    for i in dumps:
        ...
        if ndim == 2:
            for nm in range(numGrids):
                ...
                # write geometry data
                xmfFile.writelines(geometry2d % (spatialOrder*cells[0], spatialOrder*cells[1], h5, top.name,
                                             spatialOrder*cells[0], spatialOrder*cells[1], h5, top.name))
            ...
            for nm in range(numGrids):
                ...
                # write geometry data
                xmfFile.writelines(geometry3d % (spatialOrder*cells[0], spatialOrder*cells[1], spatialOrder*cells[2], h5, top.name,nm+1,
                                             spatialOrder*cells[0], spatialOrder*cells[1], spatialOrder*cells[2], h5, top.name,nm+1,
                                             spatialOrder*cells[0], spatialOrder*cells[1], spatialOrder*cells[2], h5, top.name,nm+1))
                # extract the writeNames from the excessive quotation marks
                myOutNms = eval(outNms[0])
